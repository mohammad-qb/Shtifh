...
export class TakbullResourceService {
  ...
  async PaymentHY(args: GetSigntureDTO): Promise<paymentURL> {
    ...
    const paymenturl = `${this.HybaseUrl}?action=APISign&What=SIGN&KEY=${this.HyKey}&ClientName=${firstname}&ClientLName=${lastname}&PassP=yaad.net&Masof=${this.HyMasof}&Order=${order}&Info=test-api&Amount=${amount}&UTF8=True&UTF8out=True&UserId=00000000&cell=${phone}&email=${email}&Tash=1&FixTash=False&ShowEngTashText=False&Coin=1&Postpone=False&J5=False&Sign=True&MoreData=True&sendemail=True&SendHesh=True&PageLang=ENG&tmp=9`;
